    ...
    if day in range(1, 7) and not today_is_holiday(now, holidays):
        ...
        if next_lesson_time is None and current_lesson_time is None:
            ...
            return f'Текущая пара: {schedule[current_lesson_key]["name"]}' \
                   ...
                   f'\n{"Преподаватель: " + schedule[current_lesson_key]["teacher"] if schedule[current_lesson_key]["teacher"] else ""}' \
                   f'\nПоторопись!'
        else:
            ...
            return f'Следующая пара: {schedule[next_lesson_key]["name"]} \n Через: {hours_left} ч. {minutes_left} м. \n' \
                   ...
                   f'{"Преподаватель: " + schedule[next_lesson_key]["teacher"] if schedule[next_lesson_key]["teacher"] else ""}'
    ...
    if now.isoweekday() in range(1, 7) and not today_is_holiday(now, holidays):
        ...
        for lesson in schedule.keys():
            if schedule[lesson] is not None:
                entry = f'{lessons_time[lesson]}\n{schedule[lesson]["name"]}'
                entry += f'\n{schedule[lesson]["where"]}' if schedule[lesson]["where"] else ''
                entry += f'\n{schedule[lesson]["teacher"]}' if {schedule[lesson]["teacher"]} else ''
                resp.append(entry)
        resp = '\n\n'.join(resp)
        if resp:
            return resp
